(function ($) {
	...
	(function () {
		...
		// Add Adult & childElementCount
                 var cadults = parseFloat($('#guests-count-adults-text').val());
                ...
                var cchildren = parseFloat($('#guests-count-children-text').val());
                if (isNaN(cchildren)) {
                   cchildren = 0; 
                }
                
                var croom = parseFloat($('#guests-count-room-text').val());
                if (isNaN(croom)) {
                   croom = 1; 
                }
		...
		const guestBtn = document.querySelector("#guests-input-btn"),
                    ...
                    let maxNumGuests = 15, isGuestInputOpen = false, adultsCount = cadults, childrenCount = cchildren, roomCount = croom;
                    updateValues();
		...
		function updateValues() {
			...
			roomCountEl.innerHTML = roomCount;
                        ...
                        $('#guests-count-adults-text').val(adultsCount);
                        $('#guests-count-children-text').val(childrenCount);
                        $('#guests-count-room-text').val(roomCount);
			if (adultsCount === 1) {
