...
class Hotels_model extends CI_Model {
    ...
    var $tabel_name = 'ht_hotels';
    var $tb_rooms = 'ht_rooms';
    var $hotelimgs = 'ht_hotelimgs';
    ...
    */
    public function get_records($limit, $start) {
        ...
        $query = $this->db->get($this->tabel_name); // Replace 'your_table_name' with your actual table name
    ...
    public function get_per_hotel_destination($destination)
    {
        $this->db->from ( 'ht_hotels' );
    ...
    public function get_all_hotel_images($id)
    {
        $this->db->where ( 'hot_id', $id);
        $projects = $this->db->get($this->hotelimgs)->result();
        return $projects;
    }
    ...
    */
    ...
    {
        $this->db->select ( 'h.id,c.name,c.client_image'); 
        $this->db->from ( 'ht_hotels h' );
        $this->db->join ( 'client c', 'c.id = h.merchant_id' , 'left' );
        $this->db->group_by('h.merchant_id');
        //$this->db->order_by("id", "desc");
        $this->db->limit(8);
        $query = $this->db->get ();
    ...
    public function get_per_room_price($id)
    {
        //$this->db->order_by("id", "desc");

        $this->db->select ('ht_rooms.price'); 
        ...
        if(!empty($projects->price)) {
            $price = 'YEM'.$projects->price;
        }else {
            $price = 'N/A';
        }
        return $price;
    ...
    public function get_all_rooms($id)
    {
        //$this->db->order_by("id", "desc");
        //$this->db->where('status !=','archive');
        $this->db->select ( 'ht_rooms.*,ht_rooms.id as rooms_id,roomtype.id,roomtype.name_ar,roomtype.name_en' ); 
        $this->db->from ( 'ht_rooms' );
    ...
    public function get_all_ameneties($id)
    {
        //$this->db->order_by("id", "desc");
        //$this->db->where('status !=','archive');
        $this->db->select ( 'a.name_en' ); 
        $this->db->from ( 'ht_hotel_ameneties hm' );
        $this->db->join ( 'ht_ameneties a', 'a.id = hm.amen_id' , 'left' );
        $this->db->where ( 'hm.hot_id', $id);
        $query = $this->db->get ();
        $projects = $query->result ();
    ...
    public function get_per_hotrl($id)
    {
        $this->db->from ( 'ht_hotels' );
        $this->db->where ( 'id', $id);
        $projects = $this->db->get()->row();
